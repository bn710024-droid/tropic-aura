...
// Sample NxN patch and return average RGB
...
async function run() {
  ...
  // Sample background from 4 corners (12×12 patches)
  const patchSize = 12;
  const corners = await Promise.all([
    samplePatch(pixels, width, channels, 0, 0, patchSize),
    samplePatch(pixels, width, channels, width-patchSize, 0, patchSize),
    samplePatch(pixels, width, channels, 0, height-patchSize, patchSize),
    samplePatch(pixels, width, channels, width-patchSize, height-patchSize, patchSize),
  ]);
  ...
  const t0 = 28;  // fully transparent below this distance
  const t1 = 90;  // fully opaque above this distance
  ...
  // Edge erosion (1px min-filter on alpha) to kill halo
