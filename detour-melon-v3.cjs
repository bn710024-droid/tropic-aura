...
async function run() {
  ...
  // Expansion : sat > 0.18 pour exclure eau blanche, sans coupe du bas
  ...
  // Alpha final — pas de coupe en bas
  for (let i=0;i<N;i++) {
  ...
  await sharp(px, { raw:{ width, height, channels } })
    ...
    .toFile(OUT);
  console.log("Done — melon complet sans coupe");
}
